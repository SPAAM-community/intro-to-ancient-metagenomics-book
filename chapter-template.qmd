...
- URLs should be both hyperlinked and displayed as true text.
    - For example, for [https://www.spaam-community.org/](https://www.spaam-community.org/) this should be rendered as`[https://www.spaam-community.org/](https://www.spaam-community.org/)`.
...
For example, this is a bash code block.

```{txt}
    ```{#lst-chaptertemplate-codeexample bash}
    echo "SPAAM rules!"
    ```
```
...
use a collapsable callout block with a 'Expand to see output' title immediately after the code block itself that contains.
The content of the output should also be in a triple backtick block, with the language specified.
...
Single backticks for inline code should only be used for single words such as tool names, e.g. `samtools`, or file extensions such as `.txt`.
...
```markdown
![A mock up of a possible full-book jacket for this textbook. The left hand side includes a blurb and a list of contributors, and hte SPAAM logo. The right hand side includes the title of the book, a sankey diagram, and an 'edited by' section.](assets/images/jacket.png){#fig-chaptertemplate-jacket}
```
...
All figures should be referenced in the text using the quarto label, e.g..

```markdown
 "Figure @fig-chaptertemplate-jacket shows an example figure"
```
...
: Demonstration of pipe table syntax {#tbl-chaptertemplate-exampletable1}
```

Gets rendered as in Table @tbl-chaptertemplate-exampletable1.
...
This also applies to code-block generated tables e.g. as in @tbl-chaptertemplate-examplecodetable2. For example the next code block (if wrapped in the triple-backticks with `r` syntax, not shown here).

```markdown
    
    #| label: tbl-chaptertemplate-examplecodetable2
    #| echo: false
    #| results: 'asis'
...
All references made in text will be by default rendered at the end of the document.
