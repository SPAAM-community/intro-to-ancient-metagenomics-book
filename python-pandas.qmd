...
This tutorial/walkthrough is using a [Jupyter Notebook](https://jupyter.org) for writing & executing Python code and for annotating.
...
To launch running jupyter, run the following command. This will open a browser window with jupyter running. 
...
This notebook has exactly the same code as written in this book chapter and is only a support so that it's not necessary to copy and paste everything. 
It is also possible to copy the code from this chapter into a fresh notebook file by clicking on `File` > `New` > `Notebook`. 

::: {.callout-caution}
If `student-notebook.ipynb` is not there, it is possible the working directory is not correct. Make sure that `pwd` returns `/vol/volume/python-pandas/python-pandas_lecture`.
:::

### Markdown cell syntax

Here a few examples of the syntax for Markdown cells are shown, such as making words __bold__, or _itelics. For a full list of possible syntax [click here](https://www.ibm.com/docs/en/watson-studio-local/1.2.3?topic=notebooks-markdown-jupyter-cheatsheet) for a Jupyter Notebook cheat-sheet.  
...
The **code cells** can interpret many different coding languages including __Python__ and __Bash__. The syntax of the code cells is the same as the syntax of the coding languages, in our case __python__. 
