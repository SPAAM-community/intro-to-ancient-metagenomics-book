...
The columns and rows are labelled. By default, rows are unnamed and are indexed similarly to a `series`.
...
Pandas can read in **csv** (comma separated values) files, which are tables in text format.It is called **c**sv because each value is separated from the others through a comma.
...
This can be done with the `.loc`, which **loc**ates row(s) by providing the row name and column name `[row, column]`. 
...
To remove the columns `Z_CostContact` and `Z_Revenue` and keep those changes, it is necessary to overwrite the `DataFrame`. To make sure `Pandas` understands that the its `columns` that need to be removed, the `axis` can be specified.
...
So far, all the subsetting has been based on `row names` and column names. However, in many cases, it is more helpful to look only at data that contain certain items. This can be done using conditional subsetting, which is based on Boolean values `True` or `False`.
...
You can also directly calculate the relevant statistics on numberic columns or rows you are interested in using the functions `max()`, `min()`, `mean()`, `median()` etc,.
...
Two possible actions that can be taken to deal with missing data. One is to remove the row or column using the `.dropna()` function.
This function will by default remove the row with the `NA` but with specficing the `axis=1` the column will be dropped.
...
Data is very often present in multiple tables. Think, for example, about a taxonomy table giving count data per sample. 
...
The function in Pandas that does just that is called .concat. This command combines two `DataFrames` by appending all rows or columns: `.concat([first_dataframe, second_dataframe])`.
...
This function is especially useful when you have different types of related data in separate tables. 
For example, you might have a taxonomy table with count data per sample and a metadata table in another `DataFrame`.

The pandas function `.merge()` allows you to combine these `DataFrames` based on a common column. 
...
Check out the [`Pandas` documentation](https://pandas.pydata.org/docs/reference/api/pandas.DataFrame.plot.html) for more plot `kind`s and useful syntax.
...
Now that you are familiar with python, pandas, and plotting. There are two data.tables from **AncientMetagenomeDir** which contains metadata from metagenomes. 
You should, by using the code in the tutorial be able to explore the datasets and make some fancy plots.
