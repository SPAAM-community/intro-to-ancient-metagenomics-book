...
In this chapter, we will use 10 simulated with [gargammel](https://academic.oup.com/bioinformatics/article/33/4/577/2608651) ancient metagenomic samples from @Pochon2022-hj. \
...
taxatable <- load_taxa_table("../krakenuniq/krakenuniq_abundance_matrix/krakenuniq_abundance_matrix.txt")  %>% print()
...
Currently get stuck on the command: st <- sourcetracker(otus[train.ix,], envs[train.ix]
...
You can then open the resulting `Rplots.pdf` file to see the output.
