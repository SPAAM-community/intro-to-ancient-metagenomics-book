...
In this chapter, we will use 10 simulated with [gargammel](https://academic.oup.com/bioinformatics/article/33/4/577/2608651) ancient metagenomic samples from Pochon et al. 2023 [@Pochon2022-hj]. This data under `ameta/`.
...
<!-- TODO: FIX SCRIPT, NOT WORKING -->
```bash
Rscript ../scripts/krakenuniq_abundance_matrix.R . 1000 200
```
...
```bash
cd authentication-decontamination/bowtie2
...
## Exit and save image, press 'n' when asked if you want to save your workspace image.

quit()
```

And open the `Rplots.pdf`

:::{.callout-warning}
If following during the summer school, you may need to copy the `Rplots.pdf` file to the desktop to open it in Firefox
:::
...
Then look inside the `Rplots.pdf` file for the resulting histogram.
...
:::{.callout-warning}
If following during the summer school, you may need to copy the `Fragmisincorporation_plot.pdf` and `Length_plot.pdf` file(s) to the desktop to open it in Firefox
:::
...
First move back into `authentication-decontamination/ameta` and load R.

<!-- TODO: can't test because script that generates the matrix isn't working -->
...
<!-- TODO: can't test because don't know where otus/meta come from? -->
...
TODO: @Nikolay, why not use the standalone command line version (available on biocnda): https://bioconda.github.io/recipes/sourcetracker/README.html?highlight=sourcetracker#package-package%20&#x27;sourcetracker&#x27;

Can't test because sourcetracker wasn't requested?
