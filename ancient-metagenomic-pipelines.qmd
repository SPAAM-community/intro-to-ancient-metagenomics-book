...
bibliography: assets/references/chapters/ancient-metagenomic-pipelines/references.bibtex
---
...
:::{.callout-note title="Self guided: setting up aMeta for tutorial" collapse="true"}
...
These go inside a `config.yaml` file inside `aMeta/config/`. A minimal example `config.yaml` files can look like this:
...
<!-- TODO: still necessary to document? "Next, we need to create conda sub-environments of aMeta, then manually tune a few memory related parameters of tools (Krona and Malt) included in aMeta:"" -->

Make sure you're in the `aMeta` conda environment,
...
This tutorial will re-use the data from the _de novo_ assembly chapter. If you have not-run that practical, or deleted the data, you can retrieve these with
...
We can save this `custom.config` file, and supply this to the Nextflow command with the nextflow parameter (`-c`). And with that, you can run your nf-core/mag pipeline!
