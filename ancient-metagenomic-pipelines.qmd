...
Useful files for genomics typically are in folders such as `deduplication/` which may contain f


However before you delve into these, it's normally a good idea to do a 'quality check' of the pipeline run. You can do this using the interactive MultiQC report.
...
Now we need to configure the workflow. First, we need to create a tab-delimited `samples.tsv` file inside `aMeta/config/` and provide the names of the input fastq-files. In a text editor (e.g. `nano`), write the following names paths in TSV format:
...
Next we need to write a config file. This tells aMeta where to find things such as database files and other settings.
...
:::{.callout-note}
As this is only a dummy run (due to the large-ish computational resources required for KrakenUniq), we re-use some of the resource files here. This will produce nonsense output, but is used here to demonstrate how you would execute the pipeline. 
:::
...
Make sure you're in the `aMeta` conda environment, and in the main aMeta directory with
