...
nf-core/mag covers the same major steps as you will have run if you followed the chapter on [De Novo assembly](denovo-assembly.qmd). Firstly, as with nf-core/eager, nf-core/mag do some initial sequencing QC and cleanup
...
```bash
## re-uses Alex's data, and must have CheckM databse predownloaded unpacked 
...
Make a configuration file to optimise resource requirements
...
```bash
time nextflow run nf-core/mag -r 2.3.2 -profile test,docker --input '../../denovo-assembly/*{R1,R2}.fastq.gz' --outdir ./results -c custom.config --skip_spades --skip_spadeshybrid --skip_maxbin2 --skip_concoct --binqc_tool checkm --checkm_db data/checkm_data_2015_01_16/ --ancient_dna  --binning_map_mode own --skip_prodigal --gtdb false --centrifuge_db false --kraken2_db false --skip_krona --busco_reference false --keep_phix
```
