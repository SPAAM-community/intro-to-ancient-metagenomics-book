...
Other than the raw FASTQ files, we will need a reference genome and annotation coordinates of genes present on the genome. In this case we have already downloaded the _Yersinia pestis_ (accession: GCF_001293415.1) reference genome (`.fna`) and gene feature file (`.gff`) from [NCBI Genome](https://www.ncbi.nlm.nih.gov/genome) for you so you don't have to do this. This is placed in the `reference/` directory.
...
unzipped, took just the fna and gff, then cleaned up the gff with `gffread genomic.gff` saving as the FNA file name with the gff extension

-->

With all of these, we can run the pipeline! 

A constructed eager command from within the `data/` directory will look like this:
...
::: {.callout-note}
If your nf-core/eager run hasn't finished yet, check the you can open a premade file in `ancient-metagenomic-pipelines/multiqc_report.html`
:::
...
Once opened you will see a table, and below it many table. All of these statistics can help you evaluate the quality of your data, pipeline run, and also possibly some initial results! 
...
In this tutorial we will try running the simulated data from the aMeta publication against the small test database the tool provides 
...
- @Wratten2021-es: general introduction to workflow managers
- @Fellows_Yates2021-jl: nf-core/eager publication
- @Pochon2022-hj: aMeta publication
- @Krakau2022-we: nf-core/mag publication
