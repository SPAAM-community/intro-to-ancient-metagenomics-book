...
We can assess the MCMC sampling using the program _Tracer_. _Tracer_ can read BEAST log files an generate statistics and plots for each of the sampled parameters (as well as other values that are a logged along the MCMC such as the posterior probability and the likelihood). Most importantly, _Tracer_ provides:
...
Let's now load the "beast_analysis_Y_pestis.log" file into trace (you can slide the file into the top-left panel).

::: {.callout-tip}
One can load a _BEAST2_ log file into tracer even if the analysis is still running. This allows to assess if the MCMC is running correctly or has run long enough before it's completed.
:::
