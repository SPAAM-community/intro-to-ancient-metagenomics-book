...
Do this, use `wget` or right click and save to download this Zenodo archive: [10.5281/zenodo.8413138](https://doi.org/10.5281/zenodo.8413138), and unpack
...
To perform both of these tasks, we'll use the program fastp [@Chen2018-vg; [https://github.com/OpenGene/fastp](https://github.com/OpenGene/fastp)].
...
Pavian [@Breitwieser2020-zw;[https://github.com/fbreitwieser/pavian](https://github.com/fbreitwieser/pavian)] is a web-based tool for interactive visualization and analysis of metagenomics data.
It provides a user-friendly interface for exploring taxonomic and functional profiles of microbial communities, and allows users to generate interactive plots and tables that can be customised and shared (@fig-taxonomicprofiling-pavian-interface).
...
![Screenshot of the pavian metagenomics visualisation interface, with menus on the left, a select sample and filter taxa search bar at the top, and a Sankey visualisation of the example metagenome sample](assets/images/chapters/taxonomic-profiling/pavian.png){#fig-taxonomicprofiling-pavian-interface}
...
Krona [@Ondov2011-se;[https://github.com/marbl/Krona/wiki](https://github.com/marbl/Krona/wiki)] is a software tool for interactive visualization of hierarchical data, such as taxonomic profiles generated by metagenomics tools like MetaPhlAn.
...
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-taxid-table-with-path-sep.csv")
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-taxid-table-with-path-species.csv")
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-modern-taxtable-phylum.csv")
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-modern-taxtable-species.csv")
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-sample-metadata.csv")
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-raw-multi-sample-table.csv")
...
And then run plotnine to a barplot of the mean abundance per group (@fig-taxonomicprofiling-plotnine-phylum-abundance).
...
![Stacked bar chart of ancient, non-westernised, and westernised sample groups on the X axis columns, and mean abundance percentage on the Y-axis. The legend and stacks of the bar represent different phyla each with a different colour](assets/images/chapters/taxonomic-profiling/analysis_80_0.png){#fig-taxonomicprofiling-plotnine-phylum-abundance}
...
Let's compute the **species richness**, the **Shannon index**, and **Simpson index of diversity** (@tbl-alpha-div)
...
We now make a violin plot to compare the alpha diversity for each group, faceted by the type of alpha diversity index (@fig-taxonomicprofiling-diversity-violin-plots).
...
![Three groups of violin plots of an ancient sample, westernised samples and non-westernised samples (x-axis) of the alpha diversity (y-axis) calculated for richness, shannon and simpson alpha indicies](assets/images/chapters/taxonomic-profiling/analysis_91_1.png){#fig-taxonomicprofiling-diversity-violin-plots}
...
library(gt)
# Load the data from CSV
data <- read_csv("assets/images/chapters/taxonomic-profiling/tbl-pca.csv")
...
Let's look at the variance explained by the first axes by using a scree plot (@fig-taxonomicprofiling-scree-plot).
...
![Scree plot describing the variance explained (Y-axis), for each Principal Componanent (X-axis), with a curved line from PC1 having highest variance to lowest on PC9.](assets/images/chapters/taxonomic-profiling/analysis_102_0.png){#fig-taxonomicprofiling-scree-plot}
...
Let's first look at these components with 2D plots (@fig-taxonomicprofiling-poca-1, @fig-taxonomicprofiling-poca-2)
...
![Principal Coordinate Analysis plot of PC1 (X-axis) and PC2 (Y-axis), with three groups of points in the scatter plot - blue circles of westernised data points in the bottom left, overlapping with green circles of non-westernised datapoints in the top right, and the single ancient sample as a red circle falling in between the two on the right of the overlap](assets/images/chapters/taxonomic-profiling/analysis_107_0.png){#fig-taxonomicprofiling-poca-1}
