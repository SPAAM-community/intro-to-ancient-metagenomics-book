...
The following command gets you the help of fastp
...
MetaPhlAn is a computational tool for profiling the composition of microbial communities from metagenomic shotgun sequencing data. The --help option is a common option in command-line tools that displays a list of available options and their descriptions.
...
![Pavian interface](assets/images/chapters/taxonomic-profiling/pavian.png)

#### Visualizing metaphlan taxonomic profile with [Krona](https://github.com/marbl/Krona/wiki)
...
First we install the latest version of taxopy using pip
...
Let's read the metaphlan profile table with pandas. It's a tab separated file, so we need to specify the delimiter as `\t`, and skip the comment lines of the files that start with `#`.
```python
...
To look at the head of a dataframe with pandas
...
the [TAXID](https://www.ncbi.nlm.nih.gov/taxonomy) information, so we can `drop` (get rid of) the unnecessary columns.
...
Finally, let's load the metadata, which contains the information about the modern samples.
...
Now, we need transform in the tidy format, with the `melt` function.
...
And as always, we need it in tidy format for plotnine.
...
We now make a violin plot to compare the alpha diversity for each group, faceted by the type of alpha diversity index.
