...
Jupyter notebooks are convenient and have two types of cells: **Markdown** and **Code**.
The **markup cell** syntax is very similar to **R markdown**.
...
[Pavian](https://github.com/fbreitwieser/pavian) is a web-based tool for interactive visualization and analysis of metagenomics data.
It provides a user-friendly interface for exploring taxonomic and functional profiles of microbial communities, and allows users to generate interactive plots and tables that can be customised and shared (@fig-pavian-interface).
...
[Krona](https://github.com/marbl/Krona/wiki) is a software tool for interactive visualization of hierarchical data, such as taxonomic profiles generated by metagenomics tools like MetaPhlAn.
...
(a python port of [ggplot2](https://ggplot2.tidyverse.org/reference/ggplot.html) to visualise the results).
...
To visualise this distance matrix in a lower dimensional space, we'll use a [PCoA](https://en.wikipedia.org/wiki/Multidimensional_scaling#Types), which is is a method very similar to a PCA, but taking a distance matrix as input.
...
Finally, we can also visualise this distance matrix using a clustered heatmap, where pairs of sample with a small beta diversity are clustered together (@fig-heatmap).
...
In this practical session, we first looked at how to process the raw sequencing data to focus only on the non-human reads.
We then profiled these reads with the Metaphlan3 taxonomic profiler, and looked at phylum composition, and alpha, and beta diversity with the help of PCoA and abundance heatmaps.
...
To perform quantitative analysis of taxonomic profiles, sequencind data need to be normalised for sequencing depth.
...
Once completed, the command below will remove the `/<PATH>/<TO>/taxonomic-profiling` directory **as well as all of its contents**.
