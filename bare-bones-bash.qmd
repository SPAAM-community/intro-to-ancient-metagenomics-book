...
This chapter is a condensed version of the both 'Basic' and 'Boosted' Bare Bones Bash courses that can be found in the [Bare Bones Bash website](https://barebonesbash.github.io/#/). The material is displayed here under a [CC-BY-SA 4.0](http://creativecommons.org/licenses/by-sa/4.0/) license.

![Logo of BareBonesBash, a computer monitor with a command prompt dollar symbol, BBB and a bone on the screen.](assets/images/chapters/bare-bones-bash/BareBonesBash_Icon.png){width=15% fig-align="center"}
...
![Cartoony pictures of the three authors of the BareBonesBash course. Designed by Zandra Fagernäs.](https://i.creativecommons.org/l/by-sa/4.0/88x31.png){fig-align="center"}
...
A terminal is simply a fancy window that allows us to access the command-line
interface of a computer or server (@fig-Command_line_screenshot).
...
![An example command prompt](assets/images/chapters/bare-bones-bash/Command_line_screenshot.png){#fig-Command_line_screenshot width=100%}

After opening the terminal what we will normally see is a blank screen with a
'command prompt', like the one shown above. This typically consists of our
...
The command prompt is **never** involved in any command, it is just there
to ensure we know who and where we are. When copying a 
command we should **NOT** copy the command prompt.

Often times, when looking for commands online, the commands ran will be prefaced
...
The directory `~`, stands for our **home** directory. Note that this shorthand 
will point to a different place, depending on the machine and the user.

If we want to know what the shorthand means, (here comes our first command!)
we can type in `pwd`, which stands for "**p**rint **w**orking **d**irectory". 
...
Whenever you see such a notation (i.e., triangular brackets and all caps), you must _always_ replace that whole section (including the `<>`) with whatever is on your system! You should never copy and paste this blindly!
:::
...
leading to our particular working directory.
...
* An **absolute** path will start with the deepest directory in the machine
  ...
  translates to an absolute path of our specific home directory. That is the 
  directory path we see in the output of the `pwd` command we just ran.
* Alternatively a **relative** path always begins from our working directory
  (i.e.   our current directory). Often this type of path will begin with one
...
We have just arrived to Leipzig for a summer school that is taking place at
MPI-EVA. After some questionable navigation, we find ourselves at the Bayerische
Bahnhof. Tired and disheartened, we decide to ask for help.

We see a friendly-looking metalhead (@fig-bbb_james), and decide to ask them for directions!

![A friendly-looking metalhead.](assets/images/chapters/bare-bones-bash/bbb_james.png){#fig-bbb_james width=20% fig-align="center"}
...
ALWAYS take us to MPI-EVA, but we can only apply these directions **if** we
...
Not sure how to get back to Leipzig Main Station to follow those directions, we decide to
ask someone else for directions...

Lucky for us, a friendly looking local is passing by (@fig-bbb_aida)!

![A friendly-looking local.](assets/images/chapters/bare-bones-bash/bbb_aida.png){#fig-bbb_aida width=20% fig-align="center"}
...
Each of the commands we learned above is a small program with a very
specialised functionality. Programs come in many forms and can be written
in various programming languages, but most of them share some features.
Specifically, most programs take some data in and spit some data out!
Here's how that works, conceptually:

### Datastreams

Computer programs can take in and spit out data from different _streams_ (@fig-Datastreams). By
...
![Diagram showing stdin going into the program, and two output streams from the program: stderr and stdout.](assets/images/chapters/bare-bones-bash/Datastreams.png){#fig-Datastreams width=80% fig-align="center"}
...
to our terminal to let us know when something is not going according to plan (@fig-Datastream_showcase)!

![This program (in the form of a bash script executed in the terminal) takes no input, and prints one line to the stdout and one line to the stderr.](assets/images/chapters/bare-bones-bash/Datastream_showcase.png){#fig-Datastream_showcase width=100%}

### Piping

A "pipe" (`|`) is a really useful feature of `bash` that lets us chain together
multiple commands! When two commands are chained together with a pipe, the
`stdout` of the first command becomes the `stdin` of the second (@fig-Piping)!
The `stderr` is still printed on our screen, so we can always know when things
fail.
...
![Here we can see an example of redirecting the output of the `datastreams_demo.sh` program from before. Redirecting the stdout with `>` only prints the stderr to the screen, and saves the stdout into `output.txt`. Additionally, we can redirect the stderr with `2>` into `runtime.log`, and then nothing is printed onto the screen.](assets/images/chapters/bare-bones-bash/Redirects_showcase.png){#fig-Redirects_showcase width=100%}

## Help text

We don't always have to google for documentation! Many programs come with in-built help text,
or access to online manuals right from our terminal!

- We can get a __one sentence summary__ of what a tool does with `whatis`
...
- While `man` gives us __access to online manuals__ for each tool (exit with `q`)
...
is a named container whose contents we can expand at will or change.

We can assign variables (tell the computer what do we want it to contain)
with `=` and pull their contents with `$`
...
Environment variables in bash are typically named in all capital letters. It is a good
idea to avoid using only capital letters for your variable names, so you avoid
accidentally overwriting any environment variables.
:::
...
We can also ask our computer where we have put our files, in case we forgot. To do this we can use `find`! 
...
Wow, what a mess! How would we retrieve all the files?
Thanks to our wonderful teachers we have learnt how to use find and can simply run:
...
After `-name` we have written `'*JPG*'`, this tells to `find` to search for any file that contains `JPG` in any part of its name, indicated by the `*`. The `*` are what are known as __wildcards__. To learn more on how to use them, please refer to the more complete material for this [tutorial](https://barebonesbash.github.io/#/).  

Now we have all the paths of the files that we will need!

## For loops

Until now we have seen how to run single commands on a file. But, what about when
we need to **repeat** a command multiple times on a list of things, for example
a list of files? 

To repeat an action (command) for a set of things (list, e.g. files) one needs to
employ the concept of a loop. One of the most commonly used loops, is the **for**
loop.
...
One of the most important skills we develop when coding and/or using the
command line is __how to phrase our questions__ so we can get relevant
answers out of our search engine. 
...
- ALWAYS include the name of the language in your query.
  - **BAD:**  "How to cat" (@fig-how-to-cat)
  - **GOOD:** "How to cat bash" (@fig-how-to-cat-bash)
...
![How to cat the wrong way (i.e., don't include the language in our Google search and get loads of cat pictures)](assets/images/chapters/bare-bones-bash/How_to_cat.png){#fig-how-to-cat width=45%}

![How to cat the BASH way (i.e., include the the language in our Google search and we get lots of terminal pictures! Much better, right? ...right?)](assets/images/chapters/bare-bones-bash/How_to_cat_bash.png){#fig-how-to-cat-bash width=45%}
:::

## (Optional) clean-up

It is extremely important to ALWAYS keep our directories clean from random
clutter. This lowers the chances we will get lost in our  directories, but
also ensures we can stay lazy, since <kbd>tab</kbd> completion will not keep finding
similarly named files. So let's clean up our working directory by removing all
the clutter we downloaded and worked with today. The command below will remove
the `/<PATH>/<TO>/BareBonesBash` directory **as well as all of its contents**.

:::{.callout-tip}
## Pro Tip
Always be VERY careful when using `rm -r`. Check 3x that the path we are
...
If you would like to know more about the magic of bash, you can find more commands
