...
This chapter is a condensed version of the both 'Basic' and 'Boosted' Bare Bones Bash courses that can be found in the [Bare Bones Bash website](https://barebonesbash.github.io/#/).
...
to do this in the context of bioinformatics.
We will start with how to navigate 
around a filesystem in the terminal, download sequencing files, and then to
manipulate these.
Within these sections we will also show you simple tips and
...
'command prompt', like the one shown above.
This typically consists of our
...
to ensure we know who and where we are.
When copying a 
...
When adding multi-line
commands, it is also common to preface the additional lines with a `>`.
When
...
will/should be replaced by another value.
For example, in Thiseas' command
...
* An **absolute** path will start with the deepest directory in the machine
  ...
  translates to an absolute path of our specific home directory.
  That is the 
...
* Alternatively a **relative** path always begins from our working directory
  (i.e.   our current directory).
  Often this type of path will begin with one
...
MPI-EVA.
After some questionable navigation, we find ourselves at the Bayerische
Bahnhof.
...
Programs come in many forms and can be written
...
Computer programs can take in and spit out data from different _streams_ (@fig-Datastreams).
...
multiple commands!
When two commands are chained together with a pipe, the
...
Many programs come with in-built help text,
...
In order to access to its content we will need to "unzip" it first.
...
The `*` are what are known as __wildcards__.
To learn more on how to use them, please refer to the more complete material for this [tutorial](https://barebonesbash.github.io/#/).  
...
employ the concept of a loop.
One of the most commonly used loops, is the **for**
loop.
...
clutter.
This lowers the chances we will get lost in our  directories, but
also ensures we can stay lazy, since <kbd>tab</kbd> completion will not keep finding
similarly named files.
So let's clean up our working directory by removing all
the clutter we downloaded and worked with today.
