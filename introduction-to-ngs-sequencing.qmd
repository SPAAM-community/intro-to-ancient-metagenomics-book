...
One unexpected benefits of the technology used in Illumina NGS sequencers, was that it was also ideal for sequencing _ancient_ DNA.
...
Finally we will cover some important considerations for ancient metagenomic sequencing datasets.
...
The nucleotides on one strand are connected via a sugar-phosphate 'backbone', however the two strands are held together by the interaction of the hydrogen bond pairing between the four different nucleobases on each strand ([@fig-intro-ngs-fig-2dhelix]).
...
This pairing consists of one pyrimidine and one purine that are complementary to each other.
...
This complementary-stranded characteristic of the DNA molecule is a critical component of how DNA gets replicated (or copied) within a cell.
...
This concept is important because it is the basis of how DNA sequencing works, and thus we can reconstruct the sequence of a DNA molecule for analysis.
...
Finally, the DNA is somehow separated and isolated out from the rest of the now-broken cell contents (purification).
...
Rather than well cooked, soft spaghetti structures - ancient DNA molecules are more like _extremely overcooked_ spaghetti. 
These molecules are highly degraded, broken down into very small fragments, and also often have 'damage' at the ends in the form of 'modified nucleotides' that do not represent the original sequence  (see the [Introduction to Ancient DNA chapter](introduction-to-ancient-dna.qmd) for more information).  
...
As we will find out in the next section, these short fragments of DNA is not a disadvantage for NGS sequencing, but rather a benefit.
...
These modified nucleotides include the fluorophores that emit light when excited by certain wavelengths - typically controlled by a laser.
...
Critically, as there is only a small proportion of blocking nucleotides amongst the pool of free nucleotides,the incorporation of the blocking nucleotides occurs at a slow rate and at different points during the replication process.
...
As all the molecules are of different sizes, they will move through the gel at different speeds, and therefore separate out based on the length of the molecule.
A laser is fired at the gel at regular intervals, and the light emitted from the fluorophores is detected.
...
These new sequencers were able to quickly and cheaply sequence millions of DNA molecules at once, revolutionising genetics and enabling 'accessible' genomics to a much wider group of scientists.

While a few companies emerged with competing NGS sequencers employing different techniques, such as PacBio and IonTorrent, the platform that was been by far the most successful is the one from Illumina.
These machines have been so successful, they have almost exclusively sequenced the entire corpus of ancient DNA sequencing data.
Therefore the rest of this chapter will focus just on Illumina sequencing.

### Illumina sequencing 

Illumina sequencing relies on a process termed 'Sequencing by synthesis' (SBS) but with _reversible_ terminators.
...
This works by immobilising the DNA so it stays at one location on a special type of plate.
The machine then performs a typical DNA replication, adding the flourophore modified nucleotides one at a time.
...
On this plate, we can see millions of coloured tiny dots ([@fig-intro-ngs-fig-flowcell]), that are all changing colour in realtime as each new nucleotide is added. 
...
To sequence DNA on an Illumina platform, however, we need to prepare the DNA in a specific way.
...
The Illumina platforms get around this through a process called 'clustering'.
...
To be able to offer cheaper machines, Illumina came up with alternative systems that require a fewer number of colours to be detected.
...
This characteristic of two colour-chemistry thus means we have to be aware of the type of sequencer our data was generated on, as this will require slightly different computational preprocessing, as we will see later on in the section on [Low sequence diversity](#low-sequence-diversity).
...
As we all know, biology is not like physics where everything is perfect and 'easy' [^2].
Biology likes to be messy, and things often do not stay consistent.

The same applies to sequencing - as the sequencer goes through each cycle of adding a new nucleotide to each DNA strand in a cluster, 'mistakes' will increasingly start to happen. 
...
Firstly, when measuring the particular base being recorded, all sequencers will at the same time calculate a 'base quality' that is associated with that call.
...
I.e., after bending over, the opposite flowcell oligo from the forward reverse flowcell anchor is cut so the 'end' of the original molecule is now bound to the flow cell and becomes the 'start' of the new cluster (@fig-intro-ngs-fig-pairedendsequencing).
...
Another added benefit of paired-end sequencing is that we can recover longer molecules.
...
That said, fragmented ancient DNA molecules follow a distribution of molecule lengths, and retrieval of longer reads can be useful during in analyses such as [_de novo_ assembly](denovo-assembly.qmd) for scaffolding purposes.
...
This is not something the vast majority of researchers have to do, as nowadays it happens on the sequencer itself or by the sequencing technicians and not necessary for researchers to carry out. 
...
However all variants encode a quality score of 'Q', also known as a 'Phred' score, which corresponds to the probability that the called base is _incorrect_.
...
If we are repeatedly sequencing the same DNA sequence (i.e. the amplified copies) over and over, we use up the fixed number of available clusters on the flow cell, and prevent clusters of unique sequences from forming and thus being sequenced.

This is also important analysis-wise, as will be discussed in the chapter on taxonomic profiling and genomic mapping, as we can actually artificially 'inflate' the counts of reads that come from a particular taxon (@fig-intro-ngs-fig-read-stacking).
...
While this artefact can be partially corrected during computational analysis through the process of 'deduplication', we should be careful to not overamplify our libraries in the first place as we can potentially waste a lot of money.
...
By having a stricter pattern where clusters form, it allows even tighter distribution of clusters on the flow cell without impeding each other and thus increasing the number of clusters that can be sequenced at once.
...
One workaround to this is to ligate additional (short) 'inline' barcodes directly to the DNA molecules during library preparation _prior_ to adding adapters.
...
Therefore, while it always good to quality control check the genomes going into our reference database, we should also thoroughly quality control our sequenced reads _prior_ downstream analysis.
