...
Here's the URL to the samples table of the ancient metagenome directory (learn more at https://github.com/SPAAM-community/AncientMetagenomeDir/tree/master):

```{r}
url_to_samples_table <- "https://raw.githubusercontent.com/SPAAM-community/AncientMetagenomeDir/master/ancientmetagenome-environmental/samples/ancientmetagenome-environmental_samples.tsv"
```

Load this dataset into R, into a variable "samples". The readr package can directly read data from URLs.
...
A naive assumption about this dataset might be that there is a correlation of depth and sample_age.
...
Of course we could only detect this for samples with both depth and age information. Filter the dataset to only include samples with it. And also remove samples without an archaeological site name (!= "Unknown").
...
We can't see much here, because samples with very large ages dominate the y-scale. Recreate this plot with a log-scaled y-axis.
...
This is more interesting. There may be a signal for samples below a certain age, maybe 20000 years BP. Filter the dataset to remove all samples that are older than this threshold. Store the result in a variable "samples_young".
...
With the old samples removed, there indeed seems to be a signal. Pearson's correlation coefficient is not that strong though.

```{r}
cor(samples_young$depth, samples_young$sample_age, method = "pearson")
```

Maybe what we see is mostly driven by individual sites. How many sites are there actually? Determine the number of sites in the filtered dataset.

::: {.callout-important title="Solution" collapse=true appearance="simple"}

```{r}
samples_young$site_name %>% unique() %>% length()
```

:::

Oh - that's not as much as I thought. How many samples are there per site? Calculate the number of samples per site with group_by and summarize. Sort the result table by the number of samples with arrange.
...
Prepare a bar-plot that shows this information, with the sites on the x-axis and the number of samples per site on the y-axis. The bars should be ordered by the number of samples.
...
Oh no - the x-axis labels are not well readable in this version of the plot. Create a version where they are slightly rotated.
...
Interesting! I wonder what the oldest and youngest sample are for each site. Use group_by, arrange and dplyr::slice(1, dplyr::n()) to get the oldest and youngest sample for each site in the filtered dataset.
...
The result is a bit hard to read. Select only the columns site_name and sample_age and show all rows with `print(n = Inf)`.
...
Further simplify this dataset to only one row per site (group_by) and add a column (summarize) that shows the distance between min and max age, so the age range per site.
...
So some sites have a huge age range of thousands of years, and others don't. This information is not really meaningful without the number of samples per site, though. So join the sample count per site with the age range per site to get a table with both variables.
...
Calculate the mean sampling interval by dividing the age range by the number of samples and add this information in a new column with mutate.
...
After this never-ending digression we can go back to the initial question: Is there a global relationship between sample_age and depth? Take the "samples_young" dataset and recreate the simple scatter plot from above. But now map the site_name to the point color.
...
There are a lot of samples, so the legend for the color space is annoyingly large. You can turn it off with `+ guides(color = guide_none())`.
...
It becomes very clear that we have to look at the sites separately to make sense of this data. Use faceting to split the plot into per-site subplots.
...
It's not exactly surprising that the sites operate on different scales regarding age and depth. You can add the `scales = "free"` option to facet_wrap to dynamically adjust the scaling of the subplots.
...
This looks neat for many sites, but some have too few samples. As a final exercise I suggest you remove these "single-dot" sites and recreate this plot. There are many possible ways to do this. The easiest may be to filter by the standard deviation (sd) along the age or the depth axis.
