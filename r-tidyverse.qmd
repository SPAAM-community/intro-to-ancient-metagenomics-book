...
Lecture slides and video from the [2022 edition of the summer school](https://www.spaam-community.org/wss-summer-school/#/2022/README). Note that the 2023 version features a different, more general dataset. The up-to-date, written tutorial below is the recommended version.
...
- The tidyverse is a collection of R packages with well-designed and consistent interfaces for the main steps of data analysis: loading, transforming and plotting data (<https://www.tidyverse.org>)
  - This tutorial works with tidyverse ~v2.0
  - We will learn about the packages `readr`, `tibble`, `ggplot2`, `dplyr`, `magrittr` and `tidyr`
  - `forcats` will be briefly mentioned
  - `purrr` and `stringr` are left out

- The palmerpenguins package provides a neat example dataset to learn data exploration and visualization in R (<https://allisonhorst.github.io/palmerpenguins>)

### Preparing the environment for this tutorial

- This tutorial was prepared for the SPAAM Summerschool 2023, where participants work on pre-configured virtual machines and do not have to install any additional software. The instructors will show in the beginning how to create an RStudio project and how to access the training data.
- If you're reading this later and want to tinker with the example code, then you will need to perform the following steps first:
  - Install R and RStudio for your operating system according to one of the many instructions online (e.g. [here](https://www.r-bloggers.com/2022/01/how-to-install-and-update-r-and-rstudio))
  - Download the .qmd file underlying this webpage [here](https://github.com/SPAAM-community/intro-to-ancient-metagenomics-book/blob/main/r-tidyverse.qmd) and copy it to a new directory
  - Open RStudio and create a new project in this directory with `File` -> `New Project...` -> `Existing directory`
  - Open this very file from the RStudio project, so that you can easily follow along
  - Run the `Environment preparation code` code to install necessary R package dependencies and prepare the required data used below
  
<details>
<summary>Environment preparation code</summary>
...
- A 50+kg penguin is a scary thought and we would probably remove these outliers, but let's assume they are valid observation we want to include in the plot
...
- To simplify certain analysis or plotting operations data often has to be transformed from a **wide** to a **long** format or vice versa
...
dplyr::right_join(
  ...
  y = bills, # 300 observations
  by = "id"
) %>% dplyr::arrange(id) # we arrange by id to highlight the missing
                         # observation in the peng dataset
```
...
Consider the following additional dataset with my opinions on two specific gears (in the shifting system of a car):
